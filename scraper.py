...
client = genai.Client(api_key="AIzaSyARdzjclpiOR85odSl1tLnzkDiuZKsQmUw")

TOKEN = "MTM3ODM5Njc5OTIxOTQwNDg4Mg.GI1rpz.3lfHNpoAlvXQAMr493KaETIvWN3pwIYlMqOHtw"
GUILD_ID = 1254120973096059021
...
def is_good_deal_gemini(ad):
    prompt = f"""
You are a car deal evaluator. Based on this DoneDeal listing, determine if it's a good deal:

- Title: {ad.get("title")}
- Price: €{ad.get("priceInfo", {}).get("price", 'N/A')}
- Year/Mileage info: {ad.get('metaInfo', [])}

Only reply with "Good deal" or "Not a good deal" and a short reason.
"""
...
def search_donedeal(cookies, csrf_token, params):
    ...
    if response.status_code == 200:
        ...
        for ad in ads:
            ...
            gemini_eval = is_good_deal_gemini(ad)

            embed = discord.Embed(title=title, url=f"https://www.donedeal.ie/cars-for-sale/{url}", color=0x00ff00)
            embed.add_field(name="Price", value=f"€{price}", inline=True)
            embed.add_field(name="Year", value=year, inline=True)
            embed.add_field(name="Mileage", value=f"{mileage}", inline=True)
            embed.add_field(name="Gemini Verdict", value=gemini_eval, inline=False)
